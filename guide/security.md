...
1. **Detects the runtime** — `gvisor`, `firejail`, `docker` (→ `k8s-pod` if the Kubernetes env is present), `wsl`, `macos-sandbox-exec`, or `none`. On macOS, a Seatbelt (`sandbox-exec`) profile is detected by probing: the kernel refuses to apply a nested sandbox to a process already under a restrictive profile, so a refused no-op `sandbox-exec` is the evidence (a profile that denies nothing is no boundary, and reports `none`). `systemd-nspawn`, `chroot`, and `windows-appcontainer` are accepted by the schema but not yet detectable — declare `custom` for those. Detection order matters: nested sandboxes report the outermost detectable one (a Firejail jail on a Docker host reports `firejail`).
