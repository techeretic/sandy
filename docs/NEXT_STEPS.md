...
_Last updated 2026-08-21. Read this together with `docs/DIARY.md` (chronological log) and `docs/DECISIONS.md` (resolved open questions)._

## Status

**Phase 1 is functionally complete; Phase 2 is underway.** The core library is built, tested, composed into a runnable `sandy` binary, wired into a Claude Code / Codex plugin, both conformance gates pass, and the model-engine seam is wired (138/138 tests, typecheck + build green). The launch success criterion — "zero network egress outside declared MCP endpoints" — is proven in-process and at the network level in Docker, **and the enforcer is proven runtime-agnostic**: the same config + request under Docker and Firejail produce byte-identical behavior (CI matrix). The host LLM is the engine (PL-03) and its token usage is recorded into the audit trail (AU-01) via `sandy.model.usage`. **Phase 2 has started (2026-08-21):** the model backends are built behind the `LlmEngine` seam (design §8 steps 1–2) — a llama.cpp-backed `LlamaCppEngine`, a `RemoteEngine` (SD-04), a `StubEngine` test double, the lifecycle contract (`start`/`isReady`/`close`) wired into `Sandy.close()`, the additive `llm` config fields, and engine health in `check()`. What remains is the rest of **Phase 2** (§8 steps 3–6): the autonomous loop, the loopback-only local API + `sandy serve`, the rest of the service lifecycle, and standalone conformance. See `docs/PHASE2_DESIGN.md` (build order §8, open decisions §7).
...
- The rest of the standalone service: autonomous loop (design §8 step 3), loopback-only local API + `sandy serve` (step 4), remaining service lifecycle — lazy/eager engine start + graceful shutdown (step 5), and standalone conformance (step 6, parameterize the harness config templates for a standalone `llm` block)
...
**Phase 1 is complete**, and **Phase 2 has started**: the engine backends are built (`LlamaCppEngine`/`RemoteEngine`/`StubEngine` behind the `LlmEngine` seam with its lifecycle contract, the additive `llm` config fields, engine health in `check()`, `Sandy.close()` reaping the model) — design §8 steps 1–2. Read `docs/PHASE2_DESIGN.md` first. The next work is **design §8 step 3 — the autonomous reasoning loop** (parse → run → narrate, §2.1): a **bounded** parse (retry cap ~3 with the error fed back, then a deterministic conservative fallback — never unbounded, never an unvalidated plan), model output validated against the request schema (the model proposes, the schema disposes), unit-tested against `StubEngine`. After that: **step 4** the loopback-only local API + `sandy serve` verb (bounded job store, §5), **step 5** the rest of the service lifecycle (lazy start on first `invoke()` / eager on `serve`, graceful shutdown already half-wired via `engine.close()`), **step 6** standalone conformance (parameterize the harness config templates — they hardcode `"mode":"plugin","llm":{"provider":"host"}` — for a standalone `llm` block, and add a stub-model end-to-end to the CI matrix). Open decisions that could still be settled before or during the build are in §7 (model/runtime distribution, default model, resource-limit enforcement scope, API surface scope). Note: the engine work is **uncommitted** in the working tree — commit it before starting the loop.
