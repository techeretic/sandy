...
**Phase 1 is complete** (CLI, plugin, egress conformance, sandbox conformance matrix, and the model-engine seam all pass). The next work is **Phase 2 — the standalone service** (SD-01..SD-06). **A design/ADR is drafted and reviewed in `docs/PHASE2_DESIGN.md`** — read it first. It makes the core calls (same runtime spine + autonomous loop + loopback-only REST API; llama.cpp as a subprocess behind the `LlmEngine` seam; the model runs **in-sandbox on loopback**, which is *verified* to work in all no-egress boundaries). A review pass corrected the draft's gaps: the `LlmEngine` now needs a **lifecycle contract** (`start`/`isReady`/`close`, wired into `Sandy.close()`), a **bounded** parse step, **lazy** engine start, a **bounded** API job store, and `SandyCheckReport` gains an `engine` health field. Remaining open decisions are in §7 (model/runtime distribution, default model, resource-limit enforcement scope, config field names, API surface, structured-output mechanism). Once those are approved, build in the order in §8: `LlmEngine` lifecycle + local backend (with a `StubEngine` so CI runs with no model) → config extension → autonomous loop → local API → service lifecycle → standalone conformance (parameterize the harness config templates for a standalone `llm` block).
