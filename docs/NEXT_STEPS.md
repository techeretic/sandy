...
**Phase 1 is complete** (CLI, plugin, egress conformance, sandbox conformance matrix, and the model-engine seam all pass). The next work is **Phase 2 — the standalone service** (SD-01..SD-06): a long-lived local service with a bundled 4–8B model behind the `LlmEngine.invoke()` seam (`src/engine.ts`), the local API (REST/CLI), and swappable model config (SD-04). Start by implementing a concrete `LlmEngine` (e.g. a llama.cpp binding) so the orchestrator's report pipeline can call `invoke()`; the interface, audit logging (AU-01), and fail-closed factory are already in place.
