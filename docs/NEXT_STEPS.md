...
**Phase 1 is functionally complete; Phase 2 is underway.** The core library is built, tested, composed into a runnable `sandy` binary, wired into a Claude Code / Codex plugin, both conformance gates pass, and the model-engine seam is wired (163/163 tests, typecheck + build green). The launch success criterion — "zero network egress outside declared MCP endpoints" — is proven in-process and at the network level in Docker, **and the enforcer is proven runtime-agnostic**: the same config + request under Docker and Firejail produce byte-identical behavior (CI matrix). The host LLM is the engine (PL-03) and its token usage is recorded into the audit trail (AU-01) via `sandy.model.usage`. **Phase 2 has started (2026-08-21):** the model backends are built behind the `LlmEngine` seam (design §8 steps 1–2) — a llama.cpp-backed `LlamaCppEngine`, a `RemoteEngine` (SD-04), a `StubEngine` test double, the lifecycle contract (`start`/`isReady`/`close`) wired into `Sandy.close()`, the additive `llm` config fields, and engine health in `check()`. **Steps 3–5 are done (2026-08-22):** (3) the autonomous reasoning loop (`src/standalone/loop.ts`) — parse (bounded, validated, deterministic fallback) → run → narrate — wired into `Sandy` as `sandy.loop` / `sandy ask`; (4) the **loopback-only local API** (`src/standalone/api.ts`) — a plain `node:http` REST surface over the composed `Sandy` with a **bounded** job store (serial worker, `202`+id on `POST /run`/`POST /ask`, `GET /jobs/:id`, clean `404` on eviction, `GET /health`/`/reports`/`/audit`, SSE `GET /jobs/:id/events`), off-loopback bind refused fail-closed; (5) the **service lifecycle** — `sandy serve` verb (eager `engine.start()`, graceful SIGINT/SIGTERM shutdown that closes the API, reaps the model, closes MCP, flushes audit; a dead model is a reported `degraded` state, not a crash). All unit-tested with no model/GPU. What remains is **§8 step 6 — standalone conformance** (parameterize the harness config templates for a standalone `llm` block + a stub-model end-to-end in the CI matrix). See `docs/PHASE2_DESIGN.md` (build order §8, open decisions §7).
...
- (this session) loopback-only local API + `sandy serve` + service lifecycle: `src/standalone/api.ts` (design §8 steps 4–5)
...
- **Standalone conformance (design §8 step 6, the last Phase 2 item):** parameterize the existing egress + sandbox-matrix config templates for a standalone `llm` block (they hardcode `"mode":"plugin","llm":{"provider":"host"}` at `conformance/sandbox-matrix.sh:94` and `conformance/run-docker.sh:99`) and add a stub-model end-to-end (goal → report) to the CI matrix. Steps 3–5 (loop, API, service lifecycle) are done — 2026-08-22.
...
**Phase 1 is complete**, and **Phase 2 is nearly done**: the engine backends (steps 1–2, `76fe24f`), the autonomous loop (step 3, `34b966c`), and — **this session** — the **loopback-only local API + `sandy serve` + service lifecycle** (steps 4–5) are all built, committed, and tested (163/163, typecheck + build green). Read `docs/PHASE2_DESIGN.md` first. The **only** remaining Phase 2 work is **design §8 step 6 — standalone conformance**: prove that adding a model didn't break the no-egress / cross-sandbox guarantees (SD-05/06 at the security level). Concretely: **parameterize** the existing egress + sandbox-matrix config templates for a standalone `llm` block (they hardcode `"mode":"plugin","llm":{"provider":"host"}` at `conformance/sandbox-matrix.sh:94` and `conformance/run-docker.sh:99`; the *assertions* are unchanged, only the generated config), run them against a standalone config (the in-sandbox loopback model placement, §4.0, is what makes this work in `--net=none`/`--internal`/`--network none`), and add a **stub-model end-to-end** (goal → report via `sandy ask` / the API) to the CI matrix so it runs with no model download and no GPU. The API is already loopback-only (off-loopback refused), so it needs no egress of its own — the conformance should confirm the model's loopback path and the MCP egress boundary together. Open decisions that could still be settled before or during the build are in §7 (model/runtime distribution, default model, resource-limit enforcement scope, API surface scope).
