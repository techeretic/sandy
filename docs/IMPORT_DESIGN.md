...
4. **Stage** (default)
   - **Nothing touches live config.** The validated candidate is written to `.sandy-import/<sha256>.json` — the **content hash of the fetched bytes is the filename and is pinned into the entry**. Consequences:
