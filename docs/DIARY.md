...
- **Not committed yet** (in the working tree with `src/config/schema.ts`, `src/engine.ts`, `src/index.ts`, `src/sandy.ts`, `tests/sandy.test.ts`, + new `tests/engine.test.ts`). Remaining Phase 2 per §8: **step 3** autonomous loop (parse→run→narrate, unit-tested against `StubEngine`), **step 4** loopback-only local API + `sandy serve` verb, **step 5** the rest of service lifecycle (eager engine start on `serve`, graceful shutdown), **step 6** standalone conformance (parameterize the harness config templates).
