...
- Example configs in `config/`, test suite (`npm test`) — 138 tests passing

Next: Phase 2 — the rest of the standalone service (SD-*): the autonomous reasoning loop (design §8 step 3), the loopback-only local API + `sandy serve`, and standalone conformance. Phase 1 is functionally complete (CLI, plugin, egress conformance, sandbox conformance matrix all pass), and Phase 2's model backends are built behind the `LlmEngine` seam (design §8 steps 1–2).
