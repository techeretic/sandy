...
**v0.3.0 — hardening from a real-world field test.** All 387 tests pass; typecheck + build green. **Security fix:** on a Linux host running Docker, v0.2.1 and earlier detected the bare host as a `docker` sandbox (the container detector matched other containers' mounts), so a config declaring `runtime: "docker"` could start unsandboxed — only the process's own root mount is inspected now. `sandy import` now reads **MCP Registry `server.json`** files directly (`--tools` declares the allowlist, which the registry format lacks; `--registry-source remote|package` picks the deployment when both are offered), `--apply` can add the first server, and a refused apply restores the live config byte-for-byte. `macos-sandbox-exec` is detected (a nested Seatbelt probe). Reports are never silently dropped: a report filename the configured format can't use is refused before any MCP call (exit `2`), and a report that could not be written prints `report: NOT WRITTEN` and exits `1` — in `run` and `ask`, including a failed multi-round consolidation. Markdown footnotes now pair one reference with one definition. A stdio MCP server that dies at startup now says why (its stderr tail), and a chatty server can no longer block on an unread stderr pipe. Every audit event carries a per-invocation `session` id. The v0.2.0 feature is unchanged — `sandy import <url|file|->` validates and stages an MCP server manifest from a URL, a file, or stdin — content-hash-pinned, review-first, and `--apply`-gated (see [CLI → import](guide/cli.md#import-an-mcp-server-stage-review-apply) and [`docs/IMPORT_DESIGN.md`](docs/IMPORT_DESIGN.md)). Every prior capability is unchanged: reports still render as Markdown (source of truth), HTML, **DOCX, XLSX, or PDF** (`preferences.default_report_format`), with every claim traceable to its source call — each format is a deterministic view over the same provenance-tracked claims, and the binary formats are written as byte-exact artifacts through the sandboxed File Manager. Saved report templates (`templates.json`, issue #15) re-run a named request via `sandy run <template>` or `POST /run {"template"}` — validated by the same schema + legal tool catalog as any ad-hoc request. Write-back (Q6, issue #16) is available behind an approval gate: an admin `write_allowlist` (always a subset of the read allowlist, optionally constrained per-arg) + a single-use, per-write, audited, time-bound approval via the `sandy.write` tool (+ `sandy.write.approve` / `sandy.write.revoke` for the consent flow) — the default is still read-only (every write refused). The bundled model's hard memory ceiling defaults to the service manager's cgroup; `sandbox.enforce_memory_limit: true` (issue #18) additionally enforces it in-service via a cgroup, failing closed where there is no cgroup delegation. The standalone `ask` loop can now plan **multiple** gather rounds (issue #19, opt-in via `preferences.max_planning_rounds`, default 1): after each pass the model re-plans from the results, and every follow-up round is re-validated by the same schema + legal-tool-catalog gate as the first — bounded, audited, and consolidated into a single provenance-tracked report.

- **`sandy import` — URL/file-driven MCP server configuration (v0.2.0).** Point Sandy at a URL, a file, or stdin that describes an MCP server and it produces a validated, content-hash-pinned, reviewable candidate config — staged by default, promoted only by an explicit `--apply` that re-runs the full config load. The fetched bytes are validated by the *same* fail-closed manifest schema, so nothing is legal because a URL said so. This is the first change to the egress surface: the import fetch is **the one bounded, human-confirmed, audited exception** to "zero egress outside declared endpoints" (30s / 1MB / 3-redirect caps; `import_fetch` / `import_staged` audit events). See the [security threat-model note](guide/security.md#threat-model-notes) and [`docs/IMPORT_DESIGN.md`](docs/IMPORT_DESIGN.md). v1 is deterministic-only — the LLM-transcription `--auto` path for prose pages is a documented follow-up.
- **Both modes built and conformance-proven.** Plugin mode (host LLM reasons, Sandy executes as eleven `sandy.*` MCP tools) and standalone mode (bundled llama.cpp model, autonomous plan→run→narrate loop, loopback-only `sandy serve` REST API) are complete. The launch success criterion — zero network egress outside declared MCP endpoints (with the one bounded `sandy import` exception above) — is proven in-process and at the network level in Docker, and the enforcer is proven runtime-agnostic: the same config + request under Docker and Firejail produce byte-identical behavior for **both** modes (CI matrix).
