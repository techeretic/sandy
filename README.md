...
- Example configs in `config/`, test suite (`npm test`) — 163 tests passing

Next: Phase 2 — only **standalone conformance** remains (design §8 step 6): parameterize the egress + sandbox-matrix harness config templates for a standalone `llm` block and add a stub-model end-to-end to the CI matrix, proving the no-egress / cross-sandbox guarantees hold with the in-sandbox loopback model. Phase 1 is functionally complete (CLI, plugin, egress conformance, sandbox conformance matrix all pass), and Phase 2's model backends (steps 1–2), autonomous loop (step 3), and local API + service lifecycle (steps 4–5) are built behind the `LlmEngine` seam.
